...
export const getVectorStoreChain = async () => {
  // Initialize the document loader with supported file formats
  const SOURCEPATH = path.join(process.cwd(),'public','ai','documents');
  const loader = new DirectoryLoader(SOURCEPATH, {
  ...
  const VECTOR_STORE_PATH = path.join(process.cwd(), 'public', 'ai', 'documents.index');
  // const question = "What are the suitability requirements?";
...
export default getVectorStoreChain;
